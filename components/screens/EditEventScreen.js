...
import SkeletonCategoryLoader from '../skeletonLoaders/SkeletonCategoryLoader';
...
const EditEventScreen = ({route}) => {
  ...
  const [startTime, setStartTime] = useState(
    getLocalDateFromUTC(event.datetime),
  );
  const [showTimePicker, setShowTimePicker] = useState(false);
  const [autoApproval, setAutoApproval] = useState(event.meta.auto_approval);
  const [price, setPrice] = useState(event.meta.price);
  const [description, setDescription] = useState(event.description);
  const [newAmenity, setNewAmenity] = useState('');
  const [restrictionsExpanded, setRestrictionsExpanded] = useState(true);
  const [maxParticipants, setMaxParticipants] = useState(
    event.meta.restrictions.max_participants,
  );
  const [minAge, setMinAge] = useState(event.meta.restrictions.min_age);
  ...
  const checkAndFetchGeolocation = async () => {
    if (address !== initialAddress) {
      try {
        const apiUrl = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(
          address,
        )}&key=AIzaSyDEp4eMaWFDi6psYCa4NSc3NONttlxD3Xo`;
        ...
        if (json.results && json.results.length > 0) {
          ...
          const newArea = addressComponents.find(
            component =>
              component.types.includes('sublocality_level_1') &&
              component.types.includes('sublocality'),
          );
          setCity(newCity ? newCity.long_name : '');
          setArea(newArea ? newArea.long_name : '');
        }
      } catch (error) {
  ...
  const handleCreateEvent = async () => {
    if (validateInputs()) {
      setIsLoading(true);
      try {
        await checkAndFetchGeolocation();
        const eventDetails = {
          id: event.id,
          title,
          address,
        ...
        const response = await editEventWithImages(eventDetails, images);
